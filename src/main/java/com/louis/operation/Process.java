...
public  class Process {
    ...
    public void errorTable(){
        ...
        String strErrorTable ="insert  into errorproduct(productInnerId,errorInfo,extractTime) "+
        " SELECT productInnerId,errorInfo,extractTime from "+ oldTableName +
        " where dataID in(select max(dataID) from  "+oldTableName+
        "  group BY productInnerId "+
        " ) and  errorInfo<>'' and errorInfo is not null ";
        try {
        	System.out.println(strErrorTable);
            conn = JdbcUtil.getConnection();
            stmt = conn.createStatement();
